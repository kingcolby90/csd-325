...
# To install the required module, run the following command in your terminal:
# pip install bext
    ...
    for y in range(forest['height']):
        for x in range(forest['width']):
            if forest[(x, y)] == TREE:
                ...
                bext.fg('blue')  # Display water in blue
                print(WATER, end='')
...
if __name__ == '__main__':
        main()
    # If the program is interrupted, exit:
        for x in range(forest['width']):
            for y in range(forest['height']):
                if forest[(x, y)] == FIRE:
                # Spread fire only if the neighboring tile is not water
                    for ix in range(-1, 2):
                        for iy in range(-1, 2):
                            if forest.get((x + ix, y + iy)) == TREE:
                                if forest.get((x + ix, y + iy)) != WATER:  # Firebreak prevention
                                    nextForest[(x + ix, y + iy)] = FIRE
